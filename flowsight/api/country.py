...
def _resolve_country_id(request: fastapi.Request, alpha_2: str) -> int:
    normalized = alpha_2.strip().lower()
    if normalized != request.app.default_country_alpha_2:
...
def _resolve_country_code(request: fastapi.Request, alpha_2: str) -> str:
    _resolve_country_id(request, alpha_2)
...
async def get_ssi_series(
    ...
    country_id = _resolve_country_id(request, alpha_2)
    field_ids = json.loads(field_ids) if field_ids else None
    db_data = await utils.ssi_fields_series(
        request.app.async_pool, country_id, start_date, end_date, domain_id, field_ids
    )
